...
def train_pytorch_model():
    ...
    full_img = Image.open(os.path.join('..', '..', '..', 'dataset', '128x128', 'A1702076 - 2021-07-21 18.41.42', 'macenko','patch_2.png'))

    if np.array(full_img).max() > 1: np_full_img =  np.array(full_img)/255
    # r, g, b, a = full_img.split()
    #full_img = Image.merge("RGB", (r,g,b))
    
    # Convert the PIL image into numpy array
    np_img = np.array(full_img)

    # Add an extra dim if only H, W image
    if len(np_img.shape) == 3: np_img = np.expand_dims(np_img, axis=2)

    # Re-arange the image from (H, W, C) to (C ,H ,W)
    np_img_ready = np_img.transpose((2, 3, 0, 1))
    
    # Ensure the imgs to be in [0, 1]
    if np_img_ready.max() > 1: np_img_ready = np_img_ready / 255

    img = torch.from_numpy(np_img_ready).type(torch.FloatTensor)
    img = img.to(device=device, dtype=torch.float32)



    full_mask = Image.open(os.path.join('..', '..', '..', 'dataset', '128x128', 'A1702076 - 2021-07-21 18.41.42', 'masks','mask_2.png'))
    
    # Convert the PIL image into numpy array
    np_mask = np.array(full_mask)
    ...
    # Re-arange the image from (H, W, C) to (C ,H ,W)
    np_mask_ready = np_mask.transpose((2, 3, 0, 1))
    
    # Ensure the imgs to be in [0, 1]
    if np_mask_ready.max() > 1: np_mask_ready = np_mask_ready / 255

    mask_img = torch.from_numpy(np_mask_ready).type(torch.FloatTensor)
    ...
    for epoch in range(n_epoch):
        ...
        for batch in tqdm(train_loader):
            ...
            with torch.no_grad():
                ...
                pred_mask_img, d4 = model(img)
                
                bce_loss = bse_loss(pred_mask_img, mask_img)
                dice_loss = DiceLoss(pred_mask_img, mask_img)
                loss = 0.5 * dice_loss + 0.5 * bce_loss

                

                d4_out_pil = Image.fromarray(d4)
                d4_out_pil = np.array(d4_out_pil.resize((128, 128)))
                d4_out_pil = show_cam_on_image(np_full_img, d4_out_pil, use_rgb=True)

                d4_out_pil.save('./heat_maps_2/'+str(itr_count)+"_d4_loss_"+str(loss)+".tif")
                itr_count += 1
...
if __name__ == '__main__':
    ...
    ref_image_path = os.path.join(main_data_dir, 'A1702076 - 2021-07-21 18.41.42', 'patches', 'patch_0.png')
    ...
    path_to_logs = os.path.join('..', '..', 'heat_experiments', exp_name)
    ...
    model = AttU_Net_heat_map(img_ch=n_input_channels, output_ch=n_output_channels)
